...
for (var i=0;i<20;i++){
	drop=new Drops(Math.floor(Math.random()*window.innerWidth),Math.floor(Math.random()*window.innerWidth),Math.floor(Math.random()*70));
	drops.push(drop);
}

$('document').ready(function(){
	animateInterval=setInterval(Animate,Math.floor(Math.random()*(20-10)+10));
});
...
Drops.prototype.animatedrop=function(){
	...
	if (this.radius<this.radiuslimit){
		...
		this.radioslimit=Math.floor(Math.random()*(70-40)+40);
	}
